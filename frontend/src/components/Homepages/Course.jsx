...
const Course = ({ authors, img, price, title, id, alreadyBought }) => {
  ...
  const buttonLabel =
    user && user.courses.includes(id) ? "Kupiono" : "Kup teraz";
  const isButtonDisabled = user && user.courses.includes(id) ? true : false;

  const isBoughtButtons = alreadyBought ? null : (
    <button
      className="buy-button"
      onClick={isUserLoggedHandleEvent}
